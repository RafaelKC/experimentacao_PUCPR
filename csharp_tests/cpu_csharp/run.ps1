...
# Este script deve ser executado de dentro da pasta 'cpu_csharp/'
# =============================================================================
...
$LOG_DIR = "..\logs"
$ARQUIVO_LOG = "$LOG_DIR\log_experimento_cpu_csharp_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
$BINARIO = ".\publish_win\cpu_csharp.exe"

Write-Host "========================================" -ForegroundColor Blue
Write-Host "  EXPERIMENTO: Teste CPU-bound C#" -ForegroundColor Blue
Write-Host "========================================" -ForegroundColor Blue
Write-Host ""
...
Write-Host "⚙️ Instalando dependência 'System.Diagnostics.PerformanceCounter'..." -ForegroundColor Yellow
# Adiciona o pacote NuGet
dotnet add package System.Diagnostics.PerformanceCounter
# Garante que o projeto está restaurado/pronto
dotnet restore

if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Erro na instalação ou restauração de pacotes!" -ForegroundColor Red
    exit 1
}

Write-Host "✓ Dependências configuradas. Compilando o código (Release)..." -ForegroundColor Green
# Compila e publica para o Windows (runtime win-x64)
...
if ($compileResult.ExitCode -ne 0) {
    Write-Host "❌ Erro na compilação do C#! Verifique se o SDK .NET está atualizado." -ForegroundColor Red
    exit 1
}
...
Write-Host "⚠️  IMPORTANTE: A métrica de CPU (cpu_percent) será capturada corretamente pelo PerformanceCounter do Windows." -ForegroundColor Yellow
