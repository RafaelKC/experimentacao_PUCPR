...
# Teste I/O-bound com Rust (Sem SUDO, usa arquivo grande)
# =============================================================================
...
REPETICOES=10
PAUSA_ENTRE_EXEC=5 # Aumente a pausa, pois o teste será mais longo
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  EXPERIMENTO: Teste I/O-bound RUST (Sem SUDO)${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${YELLOW}🚨 ATENÇÃO: O código Rust agora usa um arquivo de 4 GB. A primeira execução será lenta.${NC}"
echo ""
...
{ # Inicia o log
    echo "=========================================="
    echo "LOG DO EXPERIMENTO RUST I/O (4GB)"
...
echo -e "${YELLOW}⚠️  A primeira execução criará o arquivo de 4GB. Aguarde.${NC}"
...
for i in $(seq 1 $REPETICOES); do
    ...
    # Pausa entre execuções (sem comando SUDO)
    if [ $i -lt $REPETICOES ]; then
        echo -e "${YELLOW}Aguardando ${PAUSA_ENTRE_EXEC}s para próxima execução (Sem limpeza de cache)...${NC}"
